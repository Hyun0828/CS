...
- [Advanced Page Tables](#20-advanced-page-tables)
  ...
  - [역 페이지 테이블](#역-페이지-테이블)
  - [스와핑](#스와핑)
- [Swapping: Mechanisms](#21-swapping-mechanisms)
  - [스왑 공간](#스왑-공간)
  - [Present Bit](#present-bit)
  - [페이지 폴트](#페이지-폴트)
  - [메모리에 빈 공간이 없으면?](#메모리에-빈-공간이-없으면)
  - [페이지 교체 시기](#페이지-교체-시기)
- [Swapping: Policies](#22-swapping-policies)
  - [캐시 관리](#캐시-관리)
  - [최적 교체 정책](#최적-교체-정책)
  - [FIFO](#fifo)
  - [무작위 선택](#무작위-선택)
...
지금까지는 모든 프로세스의 가상 주소 공간을 물리 메모리에 전부 적재할 수 있다고 생각해왔다. 사실 그렇지는 않다. 그래서 물리 메모리가 꽉 차면 우리는 일부 페이지를 하드 디스크의 **스왑 공간**에 저장해둔다.

### 스왑 공간

스왑 공간에 페이지를 쓰는 것을 스왑 아웃, 반대로 스왑 공간의 페이지를 메모리에 적재하는 것을 스왑 인이라고 한다. 이 때 운영체제는 스왑 공간에 있는 모든 페이지들의 디스크 주소를 기억해야 한다. 그러나 스왑 공간에만 페이지를 스왑 할 수 있는 것은 아니다. 코드 영역의 페이지 같은 경우에는 코드가 저장되어 있는 파일 시스템 영역이 스왑 목적으로 사용될 수 있다.

### Present Bit

하드웨어 기반의 TLB를 사용하는 시스템을 가정하자. 만약 TLB Miss인 경우에 페이지 테이블에서 해당 PTE가 유효한지, 메모리에 적재되어 있는지를 확인한다. 메모리에 존재하면 TLB를 갱신하고 명령어를 다시 실행해 TLB Hit가 된다.

이 때 페이지가 메모리에 적재되어 있다는 것을 알려주는 비트가 Present Bit이다. 해당 비트가 0인 페이지를 접근하는 행위를 **페이지 폴트(page fault)** 라고 한다. 페이지 폴트가 발생하면 운영체제로 제어권이 넘어가 페이지 폴트 핸들러가 실행된다. 하드웨어 기반의 TLB든, 소프트웨어 기반의 TLB든 페이지 폴트 핸들러는 전부 소프트웨어로 처리된다. 

### 페이지 폴트

만약 페이지 폴트가 발생해 스왑 공간에서 페이지를 가져올 때, 해당 페이지의 디스크 주소는 어떻게 알 수 있을까? 사실 PTE에 디스크 주소를 전부 저장해둔다. 페이지를 물리 메모리로 가져와 해당 PTE의 PFN 값을 갱신하고 명령어를 재실행한다. 재실행으로 인해 TLB Miss가 발생하고 페이지 테이블을 참조해 TLB를 갱신 후 다시 명령어가 실행되어 TLB Hit가 된다.

### 메모리에 빈 공간이 없으면?

스왑 인을 하는 상황에 메모리에 빈 공간이 없다면 기존 페이지 중 하나를 선택해 스왑 아웃 시켜야 하는데 교체 페이지를 선택하는 것을 **페이지 교체 정책** 이라고 한다. 프로그램이 100,000배까지 느려질 수 있기 때문에 적절한 정책을 선택해야 한다.

TLB Miss 발생 시, 3가지의 중요한 경우가 있다.

1. 페이지가 존재하며 유효한 경우에는 페이지 테이블에서 정보를 가져와 TLB를 갱신하고 재실행한다.
2. 페이지가 유효하지만 존재하지는 않는 경우에는 페이지 폴트 핸들러가 실행된다.
3. 페이지가 유효하지 않은 경우에는 잘못된 주소를 접근한 경우라서 프로세스가 종료될 수 있다.

### 페이지 교체 시기

운영체제는 항상 어느 정도의 여유 메모리 공간을 확보해야 한다. 따라서 메모리 여유 공간에 대한 최댓값과 최솟값을 설정하는데, 여유 공간의 크기가 최솟값보다 작아지면 백그라운드 쓰레드가 동작하여 여유 공간의 크기가 최댓값에 이를 때까지 페이지를 제거한다. 이 쓰레드를 **스왑 데몬, 페이지 데몬** 이라고 부른다.

일시에 여러 개를 한 번에 교체하면 성능 개선이 가능한데 다수의 페이지들을 클러스터나 그룹으로 묶어서 한 번에 스왑 파티션에 저장하기도 한다.

## 22. Swapping: Policies

운영체제는 메모리에서 내보낼 페이지를 효율적으로 결정해야 한다. 어떤 교체 정책들이 있는지 살펴보자.

### 캐시 관리

시스템의 전체 페이지 중 일부만이 메모리에 적재될 수 있기 때문에 메인 메모리는 사실 캐시와 흡사하다. 우리는 캐시 히트를 늘리고 캐시 미스를 줄이기 위해 효율적인 교체 정책을 선택해야 한다. 캐시 히트와 미스 비율을 알면 **평균 메모리 접근 시간(Average Memory Access Time, AMAT)** 를 계산할 수 있다.

<img width="281" alt="image (5)" src="https://github.com/user-attachments/assets/0598c35f-b0b1-45b9-95b7-4b71f2a500ab" />

10개의 페이지가 있다고 하자. 또한 3번 페이지를 제외하고 모든 페이지가 메모리에 있을 때, 10개의 페이지를 순서대로 접근하면 캐시 히트율은 0.9이며 미스는 0.1이다. 메모리 접근 비용인 Tm = 100nsec, 디스크 접근 비용인 Td=10µsec이라고 할 때, AMAT는 약 1µsec가 된다. 만약 히트율이 99.9%면 대략 100배 더 빨라진다. 이와 같이 디스크 접근 횟수를 최대한으로 줄여야 한다.

### 최적 교체 정책

최적 교체 정책은 가장 나중에 접근될 페이지를 교체하는 것인데 이를 구현하는 것은 사실상 불가능에 가깝다. 현 시점에서 가장 먼 시점에 필요하게 될 페이지를 찾는 것은 미래를 예측하는 것이다. 당연히 불가능하다. 이 정책은 비교 기준으로만 사용된다.

### FIFO

교체를 할 경우에 가장 먼저 메모리에 삽입된 페이지가 교체되는 간단한 정책이다. 물론 이 방식은 많이 접근하는 페이지를 교체할 수 있어 효율이 좋진 않다.

### 무작위 선택

FIFO와 유사하게 구현하기 쉽지만 전적으로 운에 의존한다. 상황에 따라 최적의 알고리즘에 가까워질 수도 있으나 보통은 그렇진 않다.

### LRU

FIFO 또는 무작위 선택 방식처럼 단순한 정책들은 중요한 페이지를 내보낼 수 있다는 문제가 있다. 미래 예측을 위해 과거 사용 이력을 활용해보자. 가까운 과거에 접근한 페이지가 미래에 접근할 가능성이 높다고 생각하는 것이다. 페이지 교체 정책에 활용할 수 있는 데이터는 빈도수와 접근성인데 각각을 활용한 정책이 **LFU(Least-Frequently-Used)와 LRU(Least-Recently-Used)다.** 이름을 알면 어떤 정책인지 파악이 가능하다. 이와 반대되는 MFU와 MRU도 있지만 잘 쓰이진 않는다.

### 과거 이력 기반 알고리즘의 구현

LRU와 같은 알고리즘은 보통 FIFO나 무작위 선택보다 더 좋은 성능을 보인다. 그러나 이를 실제로는 어떻게 구현할 수 있을까? LRU를 예로 들어보자. 각 페이지 접근마다 언제 사용되었는지를 기록해야 한다. 즉 시간 필드를 만들어 주어야 한다. 따라서 페이지를 교체할 때는 모든 페이지들의 시간 필드를 검색하여 가장 오래된 페이지를 찾아야 하는데, 페이지 개수가 100만개면 끔찍한 성능을 보이게 된다.

### LRU 근사하기

그래서 LRU를 근사하는 방식을 대부분 사용한다. 이 개념에는 use bit 혹은 reference bit가 사용되는데 이는 PTE에서 보았듯이 해당 페이지를 참조한 적이 있는지 여부를 기록하는 비트이다. 운영체제는 LRU에 가깝게 구현하기 위해 **시계 알고리즘(Clock Algorithm)** 을 활용한다.

모든 페이지들이 환형 리스트를 구성한다고 가정하자. 시계 바늘이 특정 페이지를 가리키며 해당 페이지의 참조 비트가 1이면 0으로 바꾸고 시계 바늘을 다음 페이지로 이동시킨다. 그 후 최초로 참조 비트가 0인 페이지를 찾아 교체한다. 이렇게 순차적으로 참조 비트를 검색하는 방법 외에도 페이지를 랜덤하게 검사하는 방법도 있다.

### Dirty Page의 고려

그러나 많은 곳에서 실제 사용되는 시계 알고리즘은 이것보다 개선된 버전이다. PTE에서 Dirty Bit를 떠올려보자. 해당 페이지가 갱신되었는지 여부를 기록한 비트인데, 만약 이 값이 1이면 추후에 디스크에 변경 내용을 기록해야 하기 때문에 교체 시 추가적인 I/O가 발생한다. 이를 이용해 use bit와 dirty bit가 둘다 0인 페이지를 1순위로, use bit만 0인 페이지를 2순위로 찾아 교체한다.
