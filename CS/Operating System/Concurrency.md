...
- [Semaphores](#31-semaphores)
  ...
  - [식사하는 철학자](#식사하는-철학자)
  - [세마포어 구현](#세마포어-구현)
- [Concurrency Bugs](#32-concurrency-bugs)
  - [비 교착 상태 오류](#비-교착-상태-오류)
  - [교착 상태 오류](#교착-상태-오류)
  - [데드락 발생 조건](#데드락-발생-조건)
  - [발견 및 복구](#발견-및-복구)
...
데드락을 포함한 병행성 오류들을 살펴보자. MySQL, Apache, Mozila, OpenOffice의 코드에서 발견된 병행성 관련 오류에 대한 분석이다.

<img width="449" alt="image (8)" src="https://github.com/user-attachments/assets/e1f1d620-77c4-4fff-9707-34b0f6e7e7ab" />

위의 결과를 보면 데드락이 아닌 병행성 오류의 개수가 생각보다 많은 것을 알 수 있다. 

### 비 교착 상태 오류

비 교착 상태 오류의 대표적인 오류는 **원자성 위반 오류와 순서 위반 오류다.** 원자성 위반은 MySQL의 예제로 확인해보자.
...
이 예제에서 쓰레드1이 if문을 실행하고 쓰레드2가 값을 NULL로 바꾸면 쓰레드1로 돌아왔을 때 fputs 메소드 실행에서 문제가 발생한다. 우리는 if문과 fputs이 원자적으로 실행되는 것을 가정 했지만 이것이 깨지면 코드가 의도대로 동작하지 않는다. 물론 이런 경우에는 Lock을 추가하여 상호 배제를 구현하면 된다.
...
그 다음 순서 위반 오류를 보자. 말 그대로 실행 순서가 예상과 달라 문제가 발생하는 것이다. 
...
위 코드에서 쓰레드2가 먼저 실행되면 mThread가 초기화 되기 전이라 문제가 발생한다. 이런 순서 제어는 조건 변수를 이용하면 쉽게 해결할 수 있다.

```c
pthread_mutex_t mtLock = PTHREAD_MUTEX_INITIALIZER;
pthread_cont_t mtCond = PTHREAD_MUTEX_INITIALIZER;
int mtInit = 0;
...
원자성 또는 순서 위반 같은 비 교착 상태 오류는 락이나 조건 변수를 사용하면 해결할 수 있다. 그러나 모든 문제가 이렇게 쉽게 해결되진 않는다.

### 교착 상태 오류

복잡한 락 프로토콜을 사용하는 다수의 병행 시스템에서 데드락이 발생한다. 그럼 교착 상태는 왜 발생할까? 한 가지 이유는 코드가 많아지면서 구성 요소 간에 복잡한 의존성이 발생하기 때문이다. 가상 메모리 시스템이 디스크 블럭을 가져오기 위해 파일 시스템에 접근한다. 반대로 파일 시스템은 디스크 블럭을 메모리에 탑재하기 위해 메모리 페이지를 확보해야 하고 이를 위해 가상 메모리 시스템에 접근한다. 이렇게 자연스럽게 존재하는 순환 의존성이 교착 상태를 야기시키는 것을 방지하기 위해 대형 시스템에서 락 사용 설계는 신중해야 한다.

또 다른 이유는 **캡슐화**인데 소프트웨어 모듈화는 락과 잘 어우러지지 않는다. 예를 들어 Java의 Vector 클래스를 보면 
...
위의 코드에서는 v1, v2에 대한 락을 둘다 획득해야 하는데 다른 쓰레드가 v2.AddAll(v1)을 호출하기 라도 하면 데드락이 발생할 수 있다.

### 데드락 발생 조건

- 상호 배제 : 쓰레드가 자신이 필요로 하는 자원에 대해 독자적인 제어권을 가진다.
- 점유 및 대기 : 쓰레드가 자신에게 할당된 자원을 점유한 채로 다른 자원을 대기한다.
- 비 선점 : 자원을 소유하는 쓰레드로부터 강제로 자원을 빼앗을 수 없다.
- 환형 대기 : 락을 소유하는 쓰레드들의 순환 고리가 있다.

위 4가지 조건을 모두 만족해야 데드락이 발생하므로 1가지 조건만 해결해도 데드락이 발생하지 않는다.

가장 실용적인 방법은 **환형 대기**가 발생하지 않게 하는 것이다. 이는 락 획득 순서를 정하는 것이다. 예를 들어 L1, L2 락이 있을 때, L2를 획득하기 전에 무조건 L1을 획득해야 한다는 조건을 만들면 데드락이 발생하지 않는다. 물론 2개 이상의 락이 존재하면 전체 순서를 제어하긴 어렵다. 따라서 부분 순서를 제공하는 것도 좋은 방법이다.

원자적으로 모든 락을 단번에 획득하면 **점유 및 대기**를 예방할 수 있다. 
...
위와 같이 L1, L2에 대한 락을 획득하기 전에 전역 prevention 락을 획득하게 한다. 그러나 이 문제는 병행성이 크게 저하되는 문제가 있다.

락을 획득하려고 할 때, 이미 락이 점유된 상태면 나중에 다시 시도할 수 있게 하는 **trylock()** 루틴도 있는데 이를 통해 데드락을 예방할 수 있다. 

```java
top:
lock(L1); // 먼저 L1 락을 잡고
if (trylock(L2) == -1) { // L2 락 시도
    unlock(L1); // 실패하면 L1 풀어주고
    goto top;   // 다시 처음부터
}
```

그러나 **무한반복(livelock)** 문제가 발생할 수 있다. 2개의 쓰레드가 L1은 잡는데 L2를 못 잡게 되면 계속 코드가 반복되는 현상이다. 이것은 goto top 하기 전에 랜덤 지연 시간을 부여하면 해결할 수 있다.

**상호 배제**를 없애는 방법도 있다. 락이 필요 없는 강력한 하드웨어 명령어를 사용해 **대기없는** 자료구조가 등장했다. 락을 사용하는 대신 Compare-And-Swap 같은 명령어를 사용하면 데드락이 발생하지 않는다.

스케줄링으로 데드락을 예방할 수도 있다. 쓰레드 4개가 2개의 프로세서에서 스케줄링 된다고 해보자. 

<img width="265" alt="image (11)" src="https://github.com/user-attachments/assets/f93367aa-98dd-4136-97fd-2cd2b10a488d" />
<img width="242" alt="image (10)" src="https://github.com/user-attachments/assets/e427837a-c253-4410-92f9-140d697fe992" />

위와 같이 각 쓰레드가 락을 필요로 할 때, 오른쪽처럼 스케줄링하면 데드락이 발생하지 않는다. 그런데 이 방법은 쓰레드가 필요로 하는 락의 종류에 따라 특정 프로세서로 쓰레드가 몰려서 실행될 수도 있어 성능 하락이 발생한다. 다익스트라의 은행원 알고리즘이 이런 방법이다. 그리고 이 방법은 전체 작업에 대한 모든 지식을 알고 있어야 사용할 수 있어 보편적으로 사용되진 않는다.

### 발견 및 복구

데드락을 발견하면 복구하는 방법이다. 예를 들어 OS가 가끔 데드락이 발생하면 그냥 재부팅해버리는 방법이다. 많은 DB에선 실제로 이 방법을 사용한다.
